...
export default function Payments() {
  ...
  const handleSubmit = async (e) => {
    ...
    if (!fileBase64) return setErrorMsg("Please upload a proof of payment.");
    ...
    <div className="p-4 bg-light min-vh-100" style={{ fontFamily: 'Inter, sans-serif' }}>
      ...
      <div className="row g-3 mb-4">
        <div className="col-md-4">
          <div className="card border-0 shadow-sm p-4 h-100">
            ...
            <h3 className="fw-bold mb-0 text-success">₱{totalPaid.toLocaleString()}</h3>
          </div>
        </div>
        <div className="col-md-4">
          <div className="card border-0 shadow-sm p-4 h-100">
            <small className="text-muted d-block mb-2">Remaining Balance</small>
            <h3 className="fw-bold mb-0 text-danger">₱{remainingBalance.toLocaleString()}</h3>
          </div>
        </div>
        <div className="col-md-4">
          <div className="card border-0 shadow-sm p-4 h-100">
            <small className="text-muted d-block mb-2">Account Status</small>
            <span className={`badge rounded-pill px-3 py-2 ${hasOverdue ? 'bg-danger-subtle text-danger' : remainingBalance > 0 ? 'bg-warning-subtle text-warning' : 'bg-success-subtle text-success'}`} style={{ width: 'fit-content' }}>
              {hasOverdue ? 'Overdue' : remainingBalance > 0 ? 'Has Pending Balance' : 'Fully Paid'}
            </span>
          </div>
        </div>
      </div>
                ...
                {payments.length > 0 ? (
                  payments.map((payment) => (
                    <tr key={payment._id}>
                      ...
                      <td className="py-3 text-muted small">{payment.paymentMethod || 'N/A'}</td>
                      <td>
                  ...
                  <tr><td colSpan="4" className="text-center py-4 text-muted small">No payment records found.</td></tr>
                )}
              </tbody>
            </table>
          </div>
      ...
      <div className="card border-0 shadow-sm">
        <div className="card-body p-4">
          ...
          <p className="text-muted small mb-4">Select a pending bill and submit your proof of payment for verification.</p>
          
          {successMsg && <div className="alert alert-success py-2 text-center"><FiCheckCircle className="me-2"/>{successMsg}</div>}
          ...
          <form onSubmit={handleSubmit}>
            <div className="row g-3 mb-4">
              ...
              <div className="col-12">
                <label className="small fw-semibold mb-2">Select Bill to Pay <span className="text-danger">*</span></label>
                <select 
                ...
                >
                  ...
                  {unpaidBills.map(bill => (
                    <option key={bill._id} value={bill._id}>
                      Due: {new Date(bill.dueDate).toLocaleDateString()} - ₱{bill.amount.toLocaleString()}
                    </option>
                  ))}
                </select>
                {unpaidBills.length === 0 && <small className="text-success mt-1 d-block">You have no pending bills to pay right now.</small>}
              </div>
              ...
              {/* REFERENCE NUMBER */}
              <div className="col-md-6">
                <label className="small fw-semibold mb-2">Reference Number <span className="text-danger">*</span></label>
                <input 
                  type="text" className="form-control" placeholder="e.g. 10029348123"
                  value={refNumber} onChange={(e) => setRefNumber(e.target.value)} required disabled={unpaidBills.length === 0}
                />
              </div>
            </div>
            ...
            <div className="d-flex justify-content-end">
              <button type="submit" className="btn btn-primary px-5 py-2 fw-bold" style={{ backgroundColor: '#0ea5e9', border: 'none', borderRadius: '8px' }} disabled={submitLoading || unpaidBills.length === 0}>
